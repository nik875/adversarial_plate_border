...
class AdversarialPatchTrainer:
    ...
    def load_yolo_model(self):
        """Load YOLO model and fastanpr OCR"""
        print("Loading FastANPR model from pip package...")
        self.fast_anpr = FastANPR()
        ...
        # Note: Since fastanpr returns text directly, we can't use the same focal loss
        # This OCR is not differentiable for adversarial training
        self.detection_baseline, self.ocr_baseline = self.calculate_baseline_loss()

    def _run_fastanpr(self, image_tensor: torch.Tensor):
        """Run FastANPR on a tensor image and return aligned detections and plate objects."""
        if image_tensor.dim() == 4:
            image_tensor = image_tensor.squeeze(0)

        image_np = image_tensor.detach().clamp(0, 1).permute(1, 2, 0).cpu().numpy()
        image_np = (image_np * 255).astype(np.uint8)

        plates_batch = self.fast_anpr.run([image_np])
        plates = plates_batch[0] if plates_batch else []

        detections = []
        for plate in plates:
            x1, y1, x2, y2 = [float(v) for v in plate.det_box]
            det_conf = float(getattr(plate, 'det_conf', 0.0))
            detection = torch.tensor(
                [0.0, x1, y1, x2, y2, 0.0, det_conf],
                device=self.device,
                dtype=torch.float32,
            )
    ...
    def focal_cce_loss(
        ...
        Note: This is kept for compatibility but may not be used with fastanpr
        """
    ...
    def partial_loss(self, batch, use_ocr_baseline=True):
        ...
        # Run FastANPR detection on full patched image
        model_output, plate_results = self._run_fastanpr(prep_image)

        best_detection = None
        best_plate_result = None
        det_loss = 0.0

        for detection, plate_result in zip(model_output, plate_results):
            ...
            if self.match_detection:
                ...
                if -this_det_loss > -det_loss:  # Higher IoU*conf is better
                    ...
                    best_plate_result = plate_result
            else:
        ...
        ocr_loss = 0.0
        if best_detection is not None and best_plate_result is not None:
            ocr_text = (best_plate_result.rec_text or "")

            # Convert recognized text to loss
            # Since we can't use focal loss with logits, we'll use a simple text matching loss
            target_text = 'VRJ7774' if not self.impersonation_target else self.impersonation_target

            # Simple character-level accuracy
            correct_chars = sum(1 for a, b in zip(ocr_text.ljust(7), target_text) if a == b)
            char_accuracy = correct_chars / len(target_text)

            if self.impersonation_target:
                # For impersonation: higher accuracy = lower loss
                ocr_loss = 1.0 - char_accuracy
            else:
                # For disruption: lower accuracy = lower loss
                ocr_loss = char_accuracy
...
def test_detection_visualization(csv_path: str, output_path: str = "test_detection.png", **kwargs):
    """Test mode: visualize detections on a single image without patch using dataloader"""
    print("Running test mode - visualizing detections on sample image...")

    if 'device' not in kwargs:
        kwargs['device'] = 'cpu'  # Force CPU for visualization if not already provided
    ...
    # Run FastANPR detection on full image
    model_output, _ = trainer._run_fastanpr(prep_image)
...
def debug_patch_application(csv_path: str, patch_file: str = None,
    ...
    original_output, _ = trainer._run_fastanpr(prep_image)
    ...
    patched_output, _ = trainer._run_fastanpr(patched_image)
...
def debug_ocr_accuracy(csv_path: str, patch_file: str = None,
    ...
    def run_ocr_pipeline(use_patch=False):
        ...
        model_output, plate_results = trainer._run_fastanpr(prep_image)
        ...
        if best_plate_result is not None:
            ocr_text = best_plate_result.rec_text
            ocr_conf = float(best_plate_result.rec_conf)
        else:
            ocr_text = None
            ocr_conf = 0.0
        recognition_result = best_plate_result

        # Convert recognized text to loss
        # Since we can't use focal loss with logits, we'll use a simple text matching loss
        target_text = 'VRJ7774' if not trainer.impersonation_target else trainer.impersonation_target
        ...
        if ocr_text is not None:
            ...
            # No recognition - treat as complete failure
            ocr_loss = 1.0 if not trainer.impersonation_target else 0.0
        ...
        return {
            ...
            'ocr_recognition': recognition_result,
    ...
    results_text = f"OCR ANALYSIS RESULTS (fastanpr)\n{'='*25}\n\n"
...
def main():
    ...
    # Configuration
    CSV_PATH = "updated_control_corners.csv"
    NUM_EPOCHS = 100
    LEARNING_RATE = 0.1
