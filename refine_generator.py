...
class RefineGeneratorTrainer:
    ...
    def partial_loss(self, batch, refined_patch, use_baseline=True):
        ...
        if best_detection is not None:
            ...
            if use_baseline:
                ...
                # For baseline calculation, invert for display purposes only
        ...
        with tqdm(self.train_loader, desc=desc, leave=False) as pbar:
            with torch.no_grad():
                for batch in pbar:
                    ...
                    det_loss, ocr_loss = self.partial_loss(batch, dummy_patch, use_baseline=False)
    ...
    def compute_loss_full(self, batch: dict) -> Tuple[torch.Tensor, dict]:
        ...
        # For display: compute inverted OCR loss (1 - raw_focal_loss) for interpretability
