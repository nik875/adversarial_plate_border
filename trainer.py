...
PATCH_WIDTH  = 512
PATCH_HEIGHT = 256
DEFAULT_RTDETR_MODEL_ID = "justjuu/rtdetr-v2-license-plate-detection"
...
class AdversarialPatchTrainer:
    ...
    def apply_patch_to_image(
        ...
        if not (self.training and torch.rand(1, device=self.device).item() < 0.2):
            ...
            # Apply outside no_grad so grad flows through patch_batch
...
def main():
    ...
    parser.add_argument("--backbone-model-path", default="none",
                        help="RT-DETR model path or HF model ID for backbone feature loss model.")
