...
class PatchGeneratorNetwork(nn.Module):
    """Neural network that generates patches from latent codes"""
    def __init__(self, latent_dim: int, patch_dim: int, hidden_dims: List[int] = [256, 512, 1024]):
        super().__init__()
    ...
    def forward(self, z: torch.Tensor) -> torch.Tensor:
        ...
        Returns:
            patches_flat: [batch_size, patch_dim]
        """
        return self.network(z)


class NeuralBasisPatchTrainer:
    def __init__(self,
                 csv_path: str,
                 device: str = None,
        ...
        # Create neural network that maps latent codes to patches
        self.generator = PatchGeneratorNetwork(
    ...
    def generate_patches(self, z: torch.Tensor) -> torch.Tensor:
        """
        Generate patches from coefficients using neural network: p = NN(z)

        Args:
            z: Coefficients [batch_size, basis_dim]

        Returns:
            patches: [batch_size, 3, H, W] in [0, 1] (via sigmoid)
        """
        # Pass through neural network
        patches_flat = self.generator(z)  # [batch_size, d], already in [0, 1] via sigmoid
    ...
    def apply_patch_to_image(self, image: torch.Tensor,
        ...
        # Use kornia warp_perspective
        warped_patch = K.warp_perspective(patch_batch, M_border, dsize=dsize)

        # Create masks using dynamic image size
        patch_mask = torch.ones(batch_size, 1, self.patch_height, self.patch_width,
                                dtype=torch.float32, device=self.device)

        # Use kornia warp_perspective for masks
        warped_border_mask = K.warp_perspective(patch_mask, M_border, dsize=dsize)
        warped_plate_mask = K.warp_perspective(patch_mask, M_plate, dsize=dsize)
    ...
    def save_basis(self, epoch: int, save_dir: str = "neural_basis_activation_patches"):
    ...
    def train(self, num_epochs: int = 100, learning_rate: float = 0.01,
        ...
        for epoch in range(num_epochs):
            ...
            if val_detection_score < best_loss:
                ...
                patience_counter = 0
                self.save_basis(epoch, "best_neural_basis_activation")
            ...
            if (epoch + 1) % save_interval == 0:
                self.save_basis(epoch, "checkpoint_neural_basis_activation")
...
def main():
    ...
    # Training mode
    try:
        trainer = NeuralBasisPatchTrainer(CSV_PATH, training=True, **trainer_kwargs)
        ...
        history_df.to_csv('neural_basis_activation_training_history.csv', index=False)
        print(f"\nTraining history saved to: neural_basis_activation_training_history.csv")
        ...
        plt.tight_layout()
        plt.savefig('neural_basis_activation_training_curves.png', dpi=300, bbox_inches='tight')
        ...
        plt.tight_layout()
        plt.savefig('neural_basis_activation_sample_patches.png', dpi=300, bbox_inches='tight')

        print("\nResults saved to 'neural_basis_activation_training_curves.png' and 'neural_basis_activation_sample_patches.png'")
        print("Generator checkpoints saved in 'neural_basis_activation_patches/' directory")
