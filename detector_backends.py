...
def _select_best_from_scores_boxes(
    ...
    *,
    ...
    If any box overlaps the target (IoU >= 1e-6): return (score, box) for the
    highest IoU×score candidate, regardless of confidence threshold.
    ...
    with torch.no_grad():
        ...
        if ious.max().item() < 1e-6:
            # No box overlaps target — use proximity-weighted score for both
            # suppress and attract so gradient always flows.
...
class FasterRCNNBackend(DetectorBackend):
    ...
    def batch_predict(self, images: torch.Tensor) -> List[List[Detection]]:
        """
        Batch inference for Faster R-CNN (via List[Tensor] for proper handling).
        ...
        # Convert to list of tensors (FasterRCNN model handles this properly)
