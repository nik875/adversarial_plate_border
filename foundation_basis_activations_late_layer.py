    ...
    def _get_activations_for_patch_image(self, batch: dict, patch: torch.Tensor,
        ...
        with context:
            ...
            if self.ocr_activations is not None:
                result = self.ocr_activations.squeeze(0)  # Remove batch dim
    ...
    def train(self, num_epochs: int = 100, learning_rate: float = 0.01,
        ...
        for epoch in range(num_epochs):
            ...
            if val_detection_score < best_loss:
                ...
                patience_counter = 0
                self.save_basis(epoch, "best_foundation_activation")
            ...
            if (epoch + 1) % save_interval == 0:
                self.save_basis(epoch, "checkpoint_foundation_activation")
...
def main():
    ...
    try:
        ...
        history_df.to_csv('foundation_basis_activation_training_history.csv', index=False)
        print(f"\nTraining history saved to: foundation_basis_activation_training_history.csv")
        ...
        plt.tight_layout()
        plt.savefig('foundation_basis_activation_training_curves.png', dpi=300, bbox_inches='tight')
        ...
        plt.tight_layout()
        plt.savefig('foundation_basis_activation_sample_patches.png', dpi=300, bbox_inches='tight')

        print("\nResults saved to 'foundation_basis_activation_training_curves.png' and 'foundation_basis_activation_sample_patches.png'")
        print("Generator checkpoints saved in 'foundation_basis_activation_patches/' directory")
