...
class BasisPatchTrainer:
    def __init__(self,
        ...
        # Initialize basis matrix U with orthonormal columns
        d = 3 * self.patch_height * self.patch_width
        self.patch_dim = d

        # Create random matrix and orthonormalize via QR
        random_matrix = torch.randn(d, basis_dim, device=self.device)
        Q, R = torch.linalg.qr(random_matrix)

        # Scale to achieve desired pixel variance after p = Uz
        # For orthonormal Q: Var(p_i) ≈ scale^2 * k/d
        # We want std ≈ 0.15 (so most values in [0.2, 0.8] after +0.5 centering)
        target_std = 0.15
        scale = target_std * np.sqrt(d / basis_dim)
        self.basis_U = nn.Parameter(Q * scale)
    ...
    def generate_patches(self, z: torch.Tensor) -> torch.Tensor:
        """
        Generate patches from coefficients: p = Uz + 0.5

        Args:
            z: Coefficients [batch_size, basis_dim]

        Returns:
            patches: [batch_size, 3, H, W] clamped to [0, 1]
        """
        # p = Uz: [d, k] @ [k, batch_size]^T = [batch_size, d]
        patches_flat = z @ self.basis_U.t()  # [batch_size, d]

        # Reshape to image format
        patches = patches_flat.reshape(-1, 3, self.patch_height, self.patch_width)

        # Center at 0.5 and clamp to [0, 1]
        # This ensures patches start with values mostly in interior of [0,1]
        # so gradients flow through the clamp
        patches = torch.clamp(patches + 0.5, 0, 1)
    ...
    def compute_diversity_loss(self, patches: torch.Tensor) -> torch.Tensor:
        ...
        flat = downsampled.reshape(batch_size, -1)  # [batch_size, 3*32*64]
    ...
    def train_epoch(self, optimizer: torch.optim.Optimizer, epoch: int) -> float:
        ...
        Memory-efficient approach matching offensive_patch.py:
        - Adversarial loss: backward immediately per image (accumulates gradients, frees graph)
        - Diversity loss: computed on fresh patches sampled at end of accumulation window

        This decouples the two losses but still optimizes U for both objectives.
        """
        total_loss = 0.0
        ...
        with tqdm(enumerate(self.train_loader), desc=desc, leave=False,
            ...
            for idx, batch in pbar:
                ...
                # Scale by accumulation steps and backward immediately
                # This accumulates gradients but frees the computation graph (memory efficient)
                ...
                if step_count % update_every == 0:
                    # Sample FRESH patches just for diversity loss
                    # This is decoupled from adversarial patches but still optimizes U for diversity
                    z_diversity = self.sample_coefficients(effective_batch_size)
                    diversity_patches = self.generate_patches(z_diversity)
                    diversity_score = self.compute_diversity_loss(diversity_patches)

                    # Diversity loss term: -diversity_weight * (1/batch_size) * logdet
                    diversity_loss = -self.diversity_weight * (1.0 / effective_batch_size) * diversity_score

                    # Backward diversity loss (adds gradients to U)
                    ...
                    # Track total loss
                    avg_adv_loss = accumulation_loss / effective_batch_size
                    total_loss += avg_adv_loss + diversity_loss.item()
                    num_updates += 1

                    # Update progress bar
                    pbar.set_postfix({
                        'Loss': f"{total_loss / num_updates:.4f}",
                        'AdvLoss': f"{avg_adv_loss:.4f}",
                        'DivLoss': f"{diversity_loss.item():.4f}",
                        'Updates': num_updates
                    })

                    # Memory cleanup after update
                    del adv_loss, scaled_loss, z_diversity, diversity_patches, diversity_score, diversity_loss
                    accumulation_loss = 0.0
            ...
            if step_count % update_every != 0 and self.grad_accumulate is not None:
                remaining_steps = step_count % update_every

                # Sample fresh patches for diversity
                z_diversity = self.sample_coefficients(remaining_steps)
                diversity_patches = self.generate_patches(z_diversity)
                diversity_score = self.compute_diversity_loss(diversity_patches)
                diversity_loss = -self.diversity_weight * (1.0 / remaining_steps) * diversity_score
                diversity_loss.backward()
                ...
                avg_adv_loss = accumulation_loss / remaining_steps
                total_loss += avg_adv_loss + diversity_loss.item()
                num_updates += 1
    ...
    def save_basis(self, epoch: int, save_dir: str = "basis_patches"):
        """Save current basis state and sample patches"""
        Path(save_dir).mkdir(exist_ok=True)

        with torch.no_grad():
            # Save basis matrix
            torch.save({
                'basis_U': self.basis_U.detach().cpu(),
                'epoch': epoch,
                'basis_dim': self.basis_dim,
                'patch_size': (self.patch_height, self.patch_width)
            }, f"{save_dir}/basis_epoch_{epoch:04d}.pt")
    ...
    def train(self, num_epochs: int = 100, learning_rate: float = 0.01,
        ...
        for epoch in range(num_epochs):
            ...
            if val_detection_score < best_loss:
                ...
                patience_counter = 0
                self.save_basis(epoch, "best_basis_patches")
            ...
            if (epoch + 1) % save_interval == 0:
                self.save_basis(epoch, "checkpoint_basis_patches")
...
def main():
    parser = argparse.ArgumentParser(description='Basis Adversarial Patch Training')
    ...
    try:
        ...
        history_df.to_csv('basis_training_history.csv', index=False)
        print(f"\nTraining history saved to: basis_training_history.csv")
        ...
        plt.tight_layout()
        plt.savefig('basis_training_curves.png', dpi=300, bbox_inches='tight')
        ...
        plt.tight_layout()
        plt.savefig('basis_sample_patches.png', dpi=300, bbox_inches='tight')

        print("\nResults saved to 'basis_training_curves.png' and 'basis_sample_patches.png'")
        print("Basis checkpoints saved in 'basis_patches/' directory")
