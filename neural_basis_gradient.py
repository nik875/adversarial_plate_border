...
class NeuralBasisPatchTrainer:
    ...
    def compute_diversity_loss(self, patch_gradients: torch.Tensor) -> torch.Tensor:
        ...
        Instead of using pixel values, we use the gradients of the adversarial loss
        ...
        grads_blurred = kornia.filters.gaussian_blur2d(
            grads_jittered,
        ...
        # Apply mask to downsampled gradients
        downsampled_masked = downsampled * mask  # [batch_size, 3, H, W]

        # Flatten and L2 normalize the masked gradients
        ...
        # When gradients are diverse: det is large → log_det is positive → diversity loss is negative (rewards)
        # When gradients are similar: det ≈ 0 → log_det → -∞ → diversity loss is positive (penalizes)
        if torch.isnan(log_det):
            # NaN case: treat as singular matrix (similar gradients)
        ...
        with tqdm(enumerate(self.train_loader), desc=desc, leave=False,
            ...
            for idx, batch in pbar:
                ...
                if step_count % update_every == 0:
                    ...
                    # Note: Cannot compute gradient-based diversity during accumulation without backward pass
                    # Display previous diversity loss value
                    div_loss_scaled = last_diversity_loss * (update_every / len(accumulated_patches))
                    pbar.set_postfix({
